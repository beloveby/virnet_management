...
function turnToPage(i){
	var page = $(i).children().first().html();
	var id = $("#sidebar-list .active").first().attr("id");
		
	showContent(id, page);
}

function fowardPage(i){
	var page = $(i).siblings("ul").children(".active").children("a").html() - 1;
	var id = $("#sidebar-list .active").first().attr("id");
...
function backwardPage(i){
	var page = $(i).siblings("ul").children(".active").children("a").html() - 1 + 2;
	var id = $("#sidebar-list .active").first().attr("id");
