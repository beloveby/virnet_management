...
public class CourseInfoCDAO {
	...
	public Map<String, Object> showCourseDetail(String id, String name){
		...
		switch(id){
		case "exp-management" : break;
		case "time-management" : break;
		case "class-management" : break;
	...
	public Map<String, Object> Edit(String name){
		...
		List<Map<String, Object>> cname = this.vutil.createList("课程名称", "", "", name, "btn btn-link edit", "editable(this);", "courseName");
		...
		List<Map<String, Object>> cexp = this.vutil.createList("课程实验", "", "", explist, "mutiselect", "", "Courseexp");
	...
	public Map<String, Object> Add(){
		...
		List<Map<String, Object>> cname = this.vutil.createList("课程名称", "", "", "", "btn btn-link edit", "editable(this);", "courseName");
	...
	public boolean updateCourseExp(int courseid, String[] expNameList){
		...
		for(int i = 0; i < s; i++){
			if(!sn[i]){
				...
				dlist.add((Courseexp) this.ceDAO.getByNProperty("course.courseId", courseid + "", "exp.expId", expid + ""));
			}
		}
		...
		for(int j = 0; j < l; j++){
			if(!ln[j]){
				...
				courset.setCourseId(courseid);
				ce.setCourse(courset);
	...
	public List<Map<String, Object>> getCourseExp(int id){
		...
		List<Courseexp> celist = this.ceDAO.getListByProperty("course.courseId", id);
		
		int size = celist.size();
		for(int i = 0; i < size; i++){
			Map<String, Object> map  = new HashMap<String, Object>();
			map.put("name", this.eDAO.getExpName(celist.get(i).getExp().getExpId()));
			map.put("onclick", "showDetail('" + this.eDAO.getExpName(celist.get(i).getExp().getExpId()) + "', 'exp');");
	...
	public List<Map<String, Object>> getCourseClass(int id){
		...
		List<Class> clist = this.classDAO.getListByProperty("course.courseId", id);
	...
	public boolean IsCourseExp(int courseid, int expid){			
		if(this.ceDAO.getByNProperty("course.courseId", courseid + "", "exp.expId", expid + "") == null){
			return false;
		}
		else{
			return true;
